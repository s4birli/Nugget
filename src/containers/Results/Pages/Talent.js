...
import { FilterCellList } from './FilterCellList';
...
const { updateCandidate, getCandidate, deleteCandidate } = candidateActions;
const { getPipelinesRequest } = adminActions;

const INSIGHT_TAB = 1;

class TalentResult extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      showFilteredPipeline: false,
      currentFilteredPipelineId: -1,
      currentFilteredPipelineColor: '',
      filterlist: {},
    };
    this.tableData = {
      fullname: '',
  ...
  /**
   * Get currentSearchPipelineID and setState
   */
  ...
  /**
   * Return filtered result by filtervalue which is from children component
   */
  ...
  handleClickIcon = pid => (cId, type) => {
    const { updateCandidate, getChallenge, deleteCandidate } = this.props;
    const { challengeId } = this.state;
    if (type != 4) {
      updateCandidate({
        challengeId,
  ...
  handleFilteredPipeline = (pid, pcolor) => (showFilterModal, filterlist) => {
    this.setState({
      showFilteredPipeline: true,
      currentFilteredPipelineId: pid,
      currentFilteredPipelineColor: pcolor,
      filterlist,
    });
  };
  ...
  renderPipeline = pipelines => {
    ...
    const { showFilteredPipeline } = this.state;
    if (showFilteredPipeline === false) {
          ...
          currentSearchPipelineId === pipeline._id
            ? users.filter(
                user =>
                  ...
                  user.lastname.toLowerCase().includes(this.state.filterValue),
              )
            : users;
        return (
          <Grid item xs={12} md={12} sm={12} key={index}>
            <PipeLine
              ...
              handleUpdateTitle={this.handleUpdatePipelineTitle(pipeline._id)}
              benchmark={pipeline.benchmark}
              isFullScreenPipeline={isFullScreenPipeline}
              copyLink={this.handleCopyLink(pipeline._id, pipeline.type)}
              pid={index}
              pipeline={pipeline}
              expanded
              onClickIcon={this.handleClickIcon(pipeline._id)}
              onChangeFilterValue={this.handleChangeFilterValue(pipeline._id)}
              onShowPipelineByFilter={this.handleFilteredPipeline(
                pipeline._id,
                pipeline.color,
              )}
            />
      ...
      const {
        ...
        roleFilter,
        skillFilter,
        value, //value is date range which is selected at filter list
        isAuthorized,
      } = filterlist;
      const curPipeline = pipelines.filter(
        pipeline => pipeline._id == currentFilteredPipelineId,
      );
      const { users } = curPipeline[0];
      const pipeline = curPipeline[0];
      let filteredData;
      /**
       *
       *      Make a list that is sent from talent to datatable for Filtervalues
       *
       */
      let availableFilterList = FilterCellList;
      // Set date ragne
      const { start, end } = value;
      const from = new Date(start).getTime();
      const to = new Date(end).getTime();
      // To complete selectedCategoryList
      if (eduLevelFilter) {
        availableFilterList[0].isShow = true;
      ...
      if (expYearFilter) {
        availableFilterList[1].isShow = true;
        selectedCategoryList.push({
          value: expYearFilter,
          field: 'expYears',
          type: 'equal',
      ...
      if (roleFunctionFilter) {
        availableFilterList[2].isShow = true;
        selectedCategoryList.push({
          field: 'roleFunction',
      ...
      if (roleLevelFilter) {
        availableFilterList[3].isShow = true;
        selectedCategoryList.push({
      ...
      if (roleFilter) {
        availableFilterList[4].isShow = true;
      ...
      if (skillFilter) {
        availableFilterList[5].isShow = true;
      ...
      if (value.start) {
        availableFilterList[6].isShow = true;
        selectedCategoryList.push({
          value: value,
          type: 'daterange',
          field: 'createdDate',
      ...
      if (isAuthorized != undefined) {
        availableFilterList[7].isShow = true;
        selectedCategoryList.push({
          value: isAuthorized,
          type: 'check',
      ...
      const checkArrayFunc = param => {
        return Array.isArray(param);
      };
      filteredData = users;
      selectedCategoryList.map(filterItem => {
        filteredData = filteredData.filter(user => {
          //check date range
          let dateFilterFlag = true;
          if (value) {
            const userCreatedDate = new Date(user['createdDate']).getTime();
            if (!(userCreatedDate >= from && userCreatedDate <= to)) {
              dateFilterFlag = false;
            }
          }
          //check authorized

          const userItems = user[filterItem.field];
          const filterValues = filterItem.value;
          if (filterItem.type == 'equal') {
            return (
              filterValues.findIndex(filterValue => {
                if (checkArrayFunc(userItems)) {
                  return (
                    userItems.findIndex(userItem => {
                      return userItem === filterValue;
                    }) >= 0
                  );
                } else {
                  return userItems === filterValue;
                }
              }) >= 0
            );
            ...
            return dateFilterFlag;
          } else {
            let authorizedFlag = true;
            if (isAuthorized != user['authorization_status'])
      ...
      return (
        <Grid item xs={12} md={12}>
          <PipeLine
            ...
            userData={filteredData}
            width={12}
            color={currentFilteredPipelineColor}
            onRemove={this.handleRemovePipeline(pipeline._id)}
            changeBenchmark={() => {
              this.handleChangeBenchmark(pipeline._id);
            }}
            ...
            filterCells={availableFilterList}
          />
        </Grid>
      );
    }
  ...
  render() {
    Modal.setAppElement(this.el);
    const { curChallenge, user, curChallenge: challenge } = this.props;
    const { selected, challengeId } = this.state;
    let pipelines;
    if (curChallenge) {
          ...
          <Grid container spacing={32}>
            ...
            {/* <Grid className="challenge" item xs={12} md={6}>
              <TextField
                ...
                value={this.context.t(challenge.test_name)}
                InputProps={{
