...
const TALENT_TAB = 0;
const OPTIMUS_CHALLENGE_ID = '5d50a2e27111090c9cf0ac9e';
...
class InsightResult extends Component {
  ...
  componentDidMount = async () => {
    ...
    if (challengeId === OPTIMUS_CHALLENGE_ID) {
      await this.initData();
    }
  ...
  render() {
    ...
    const dbPipelines = this.getDBPipelines();
    console.log('db pipelines ', dbPipelines);
    const employeePipeline = dbPipelines[0];
    const candidatePipeline = dbPipelines[1];
    let progressStep = 1; // set step to 1 as default

    if (challengeId === OPTIMUS_CHALLENGE_ID) {
      progressStep = 3; // set step to 3 when all data is ready
    } else {
      dbPipelines.forEach(item => {
        if (item.users && item.users.length >= 50) {
          progressStep = 2; // set step to 2 when users are over 50
        }
      });
    }
    ...
    const { curChallenge: challengeData } = this.props;
    const challengeTitle = challengeData.test_name || challenge.test_name;
